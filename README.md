...
1. Technology Stack
Frontend: React.js, Redux (for state management), Tailwind CSS/Material-UI Backend: Node.js, Express.js Database: MongoDB with Mongoose ORM Authentication: JWT (JSON Web Tokens), Google OAuth Storage: Cloudinary/AWS S3 (for image & video uploads) Payment Integration: Stripe/PayPal (for ad boosting) Hosting & Deployment: Vercel/Netlify (Frontend), AWS/Render/DigitalOcean (Backend), MongoDB Atlas (Database)
2. System Architecture
Frontend (React.js): UI for buyers, sellers, and admins.


Backend (Node.js + Express.js): Handles authentication, ad management, user management, and payment processing.


Database (MongoDB): Stores user accounts, advertisements, and transaction data.


Cloud Storage (Cloudinary/AWS S3): Handles image and video uploads.


Authentication (JWT + OAuth): Manages secure login, registration, and password recovery.
