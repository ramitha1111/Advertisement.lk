...
const SiteSettings = () => {
    ...
    const fetchCurrentSettings = async () => {
        try {
            ...
            setLogoSettings(prev => ({
                ...prev,
                currentLogo: response.data.logo
            }));
        } catch (err) {
    ...
    const handleLogoSubmit = async (e) => {
        ...
        try {
            ...
            const errorMessage = err.response?.data?.message || 'Failed to update logo. Please try again.';
    ...
    const handleAddCategory = async () => {
        ...
        try {
            setLoading(true);
            await api.post('/site-settings/homepage-categories', {
    ...
    const handleRemoveCategory = async (categoryId) => {
        try {
            setLoading(true);
            await api.delete(`/site-settings/homepage-categories/${categoryId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
            ...
            const errorMessage = err.response?.data?.message || 'Failed to remove category. Please try again.';
            setError(errorMessage);
        } finally {
            setLoading(false);
        }
    ...
    const handleDrop = async (e, dropIndex) => {
        ...
        try {
            await api.put('/site-settings/homepage-categories/reorder', {
                categoryOrders
            }, {
                headers: {
                    'Authorization': `Bearer ${token}`
            ...
            const errorMessage = err.response?.data?.message || 'Failed to reorder categories';
            setError(errorMessage);
            ...
            <div className="space-y-6">
                ...
                <div>
                    ...
                    {homepageCategories.length === 0 ? (
                        <div
                            className="bg-gray-50 dark:bg-gray-700 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center">
                        ...
                        <div className="space-y-2">
                            {homepageCategories.map((category, index) => (
                                ...
                                >
                                    <div className="flex items-center space-x-4">
                                        <div
                                            className="flex items-center justify-center w-8 h-8 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-full font-medium text-sm">
                                            {index + 1}
                                        </div>
                                ...
                                >
                                    {[1, 2, 3, 4, 5].map(num => (
                                        <option key={num} value={num}
                                                disabled={homepageCategories.some(cat => cat.order === num)}>
                                            {num}
                                        </option>
                                    ))}
                                </select>
